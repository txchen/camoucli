...
- Current status: the scaffold, installer, daemon, session/tab manager, IPC, browser version switching, presets, structured JSON errors, and doctor diagnostics are in place; unit and integration tests exist in `tests/`, and a real Camoufox install/launch smoke test has been exercised locally.
